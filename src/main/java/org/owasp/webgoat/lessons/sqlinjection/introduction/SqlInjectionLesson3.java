...
public class SqlInjectionLesson3 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQuery(String queryParam) {
    try (Connection connection = dataSource.getConnection()) {
      String safeQuery = "UPDATE some_table SET some_column = ? WHERE some_other_column = ?";
          try (PreparedStatement preparedStatement = connection.prepareStatement(safeQuery)) {
        preparedStatement.setString(1, queryParam);
            preparedStatement.setString(2, "some_value"); // Replace "some_value" with actual logic or parameter
