...
public class SqlInjectionLesson9 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryIntegrity(String name, String auth_tan) {
    ...
    String queryInjection = "SELECT * FROM accounts WHERE name = ? AND auth_tan = ?";
    ...
    while (resultSet.next()) {
      output.append(resultSet.getString("account_data")).append("\n");
      }
      }
      } catch (SQLException e) {
      // Handle SQL exception
      return AttackResult.error("Database error: " + e.getMessage());
      }
