...
public class SqlInjectionLesson9 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryIntegrity(String name, String auth_tan) {
    ...
    String queryInjection =
        "SELECT * FROM users WHERE name = ? AND auth_tan = ?";
            try (PreparedStatement statement = connection.prepareStatement(queryInjection)) {
            statement.setString(1, name);
            statement.setString(2, auth_tan);
            try (ResultSet resultSet = statement.executeQuery()) {
    while (resultSet.next()) {
      output.append(resultSet.getString("column_name")).append("\n");
      }
      }
      } catch (SQLException e) {
      e.printStackTrace();
      return AttackResult.failed("Error: " + e.getMessage());
      }
